...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _ux_host_class_meige_endpoints_get                   PORTABLE C     */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*    endpoints. The MEIGE USB device has multiple interfaces.            */
...
UINT _ux_host_class_meige_endpoints_get(UX_HOST_CLASS_MEIGE *meige)
{
    ...
    for (endpoint_index = 0; endpoint_index < meige->ux_host_class_meige_interface->ux_interface_descriptor.bNumEndpoints; endpoint_index++)
    {
        /* Get interface endpoint. */
        status = _ux_host_stack_interface_endpoint_get(meige->ux_host_class_meige_interface, endpoint_index, &endpoint);
        ...
        if (status == UX_SUCCESS)
        {
                ...
                ((endpoint->ux_endpoint_descriptor.bmAttributes & UX_MASK_ENDPOINT_TYPE) == UX_BULK_ENDPOINT))
            {
                ...
                meige->ux_host_class_meige_bulk_out_endpoint =  endpoint;
                break;
            }
        }
    }
    ...
    if (meige->ux_host_class_meige_bulk_out_endpoint == UX_NULL)
    {
        /* Error trap. */
        _ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_CLASS, UX_ENDPOINT_HANDLE_UNKNOWN);

        /* If trace is enabled, insert this event into the trace buffer. */
        UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_ENDPOINT_HANDLE_UNKNOWN, meige, 0, 0, UX_TRACE_ERRORS, 0, 0)
    ...
    {                        
        ...
        if (status == UX_SUCCESS)
        {
                ...
                ((endpoint->ux_endpoint_descriptor.bmAttributes & UX_MASK_ENDPOINT_TYPE) == UX_BULK_ENDPOINT))
            {
                ...
                /* We have found the bulk endpoint, save it. */
                meige->ux_host_class_meige_bulk_in_endpoint = endpoint;
    ...
    if (meige->ux_host_class_meige_bulk_in_endpoint == UX_NULL)
    {
        /* Error trap. */
        _ux_system_error_handler(UX_SYSTEM_LEVEL_THREAD, UX_SYSTEM_CONTEXT_CLASS, UX_ENDPOINT_HANDLE_UNKNOWN);
