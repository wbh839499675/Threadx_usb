  ...
  switch (phost->gState)
  {
      ...
      else
      {
        if (phost->Timeout > USBH_DEV_RESET_TIMEOUT)
        {
          ...
          if (phost->device.RstCnt > 3U)
          {
            ...
            printf("USB Reset Failed, Please unplug the Device.");
    ...
    case HOST_ENUMERATION:
      ...
      if (status == USBH_OK)
      {
        ...
        {
          printf("This device has only 1 configuration.");
      ...
      else
      {
        ...
        if (phost->pActiveClass != NULL)
        {
          if (phost->pActiveClass->Init(phost) == USBH_OK)
          {
            ...
            printf("%s class started.", phost->pActiveClass->Name);
          ...
          {
            phost->gState = HOST_ABORT_STATE;
            printf("Device not supporting %s class.", phost->pActiveClass->Name);
          }
        }
        else
        {
    ...
    case HOST_CLASS_REQUEST:
      ...
      if (phost->pActiveClass != NULL)
      {
        ...
        {
          phost->gState = HOST_ABORT_STATE;
          printf("Device not responding Please Unplug.");
        }
        else
        {
          /* .. */
